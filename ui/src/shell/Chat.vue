<script setup>
// Top-left chat (DESIGN §30.3, §37.6). One store-owned open flag, one idle timeout;
// no polling. The server owns routing, permissions and proximity opacity.
// Kit skin: the composer is the kit BOX (`core-inputbox` + `core-inputbox--sm`, the `is-focused`
// state toggled by hand) around a raw <input> — the byte clamp, the caret (`selectionStart`,
// `setSelectionRange`) and the IME rules all need the element itself, which CoreInput does not
// hand out. Channel button, suggestion panel and the hint footer ARE kit components.
...
// The suggestion footer, in the kit's hint vocabulary: caps + caption.
const HINTS = [{ keys: ['↑', '↓'], label: 'select' }, { keys: 'Tab', label: 'complete' }]
...
<template>
  <section class="chat fixed z-[30] left-[18px] top-[18px] text-ui-sm text-fg"
    ...
    <div ref="feedEl" class="feed core-scroll" :class="{ 'feed-visible': feedVisible, 'feed-reading': open }"
      ...
      <div v-for="line in lines" :key="line.id" class="line" :style="{ opacity: lineOpacity(line) }">
        ...
        <template v-else>
          <span v-if="line.tag" class="tag font-semibold" :style="{ color: colorOf(line) }">{{ line.tag.trim() + ' ' }}</span><span
            class="name font-semibold" :class="{ uppercase: line.kind === 'scream' }"
    ...
    <div v-if="open" class="composer mt-[7px]">
      <!-- The kit's box look, state included: `is-focused` is what the kit lights the well with,
           and the raw <input> keeps the caret API the byte clamp and the completion need. -->
      <div class="row core-inputbox core-inputbox--sm" :class="{ 'is-focused': focused }" data-core-blur>
        <CoreButton class="chan-btn flex-none px-0" variant="ghost" size="sm" tabindex="-1"
                    :title="`${activeChannel.description || activeChannel.label || activeChannel.id} · Ctrl+Tab to switch`"
                    @mousedown.prevent @click="cycleChannel()">{{ activeChannel.label || activeChannel.id }}</CoreButton>
      ...
      <!-- The suggestion popup is a kit panel; the rows wear the kit's menu row, so the selected
           command gets the same dissolving coral a menu entry has (§37.1). -->
      <CorePanel v-if="matches.length" variant="solid" padding="none" tag="div"
                 class="command-help mt-[4px] overflow-hidden">
        <ul id="chat-commands" ref="suggestionsEl" class="command-list core-menu core-menu--sm is-fade core-scroll list-none m-0 p-0"
            role="listbox" aria-label="Available commands">
          <li v-for="(command, index) in matches" :id="`chat-command-${index}`" :key="command.command"
              class="command-option core-menu__item flex-col items-start justify-center gap-0 py-[7px]"
              :class="{ selected: selected === index, 'is-active': selected === index }" role="option"
              :aria-selected="selected === index" @mousedown.prevent="acceptCommand(command)">
            <div class="signature flex flex-wrap gap-[6px] font-mono text-[12px]"><span class="font-medium">{{ command.command }}</span><span
              v-for="(param, p) in command.params" :key="p" class="param opacity-70">{{ param.name }}</span></div>
            <p v-if="command.description" class="command-description opacity-70">{{ command.description }}</p>
          </li>
        </ul>
        <template #footer>
          <CoreKeyHints :items="HINTS" bare size="sm" align="between"
                        class="command-keys w-full px-[11px] pt-[5px] pb-[7px]">
            <template #end><span class="core-label">{{ selected + 1 }} / {{ matches.length }}</span></template>
          </CoreKeyHints>
        </template>
      </CorePanel>

      <CorePanel v-else-if="context.command" variant="solid" padding="sm" tag="div"
                 class="argument-help command-help mt-[4px]">
        <div class="signature flex flex-wrap gap-[6px] font-mono text-[12px]"><span class="opacity-70">{{ context.command.command }}</span><span
          v-for="(param, index) in context.command.params" :key="index" class="param"
          :class="index === context.argument ? 'param-active text-accent underline underline-offset-[3px]' : 'opacity-70'"
          :aria-current="index === context.argument ? 'step' : undefined">{{ param.name }}</span></div>
        <p v-if="currentParam" id="chat-argument-help" class="argument-description opacity-70">
          {{ currentParam.help || currentParam.name }}<span class="param-type ml-[8px] opacity-70">{{ currentParam.type || 'string' }} · {{ currentParam.optional || currentParam.name.startsWith('[') ? 'optional' : 'required' }}</span>
        </p>
        <p v-else class="command-description opacity-70">{{ context.command.description }}</p>
      </CorePanel>
    </div>
  </section>
</template>
...
<style scoped>
/* §37.6: everything that PAINTS the chat is a kit class or a kit component — the box look on the
   composer, the panel under the suggestions, the menu row under the selected command, the kit's
   thin scrollbar through `core-scroll`. What is left here is geometry plus the one thing the kit
   has no answer for: the feed is chrome-less text lying on the world, kept readable by a shadow
   rather than by a plate behind it (the §30.3 look), and its fade is a visibility transition the
   store drives. */
...
.command-list { max-height: min(230px, 30vh); }
